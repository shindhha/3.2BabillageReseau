...
def initDb():
    if (not(os.path.exists(cheminBd) and os.path.isfile(cheminBd))):
        ...
        cursor.execute(""" CREATE TABLE clefs (nom TEXT, clef TEXT, ip TEXT, port TEXT,ks TEXT, CONSTRAINT pk_clefs PRIMARY KEY (nom)) """) # requete de creation de la table
...
def ajout(nom,cle,connection):
    ...
    port = connection[1]
    cursor.execute("INSERT INTO clefs VALUES (?,?,?,?,NULL)", (nom,cle,ip,port,)) # si pas créer creation de l'utilisateur et ajout de sa clef
    connectionBd.commit()
    connectionBd.close()
